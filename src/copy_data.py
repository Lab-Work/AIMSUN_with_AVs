...
for sce, seed in sce_seeds:
	...
	for run in range(0, 10):
		...
		for p in prop:
			for m in mdl:
				src = result_dir + 'Estimation_corrected__{0}/Estimation{1}_PR_{2}_Seed{3}_{4}.npy'.format(run, p, sce, seed, m)
		...
		if run == 0:
			# copy the true state
			src_true = result_dir + 'Estimation_corrected__{0}/TrueDensity_PR_{1}_Seed{2}_1st.npy'.format(run, sce, seed)
