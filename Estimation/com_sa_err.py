...
def main(argv):
    ...
    directorySave = os.getcwd() + '/Result/SA/'
    directoryLoad = None
    ...
    for sa in sa_sces:
        ...
        directoryLoadPrefix = dirs_load[sa]
        ...
        save(directorySave+'sa_error_{0}_1st_cf.npy'.format(sa), error_array_1st_cf)
        save(directorySave+'sa_error_{0}_2nd_all.npy'.format(sa), error_array_2nd_all)
        save(directorySave+'sa_error_{0}_2nd_ff.npy'.format(sa), error_array_2nd_ff)
        save(directorySave+'sa_error_{0}_2nd_cf.npy'.format(sa), error_array_2nd_cf)
        ...
        scatter_err(directorySave, PRsetTest, sensorLocationSeed, runs,
                    ...
                    error_array_2nd_all, '2nd model', colors, 'Mean estimation error', 'err_summary_{0}_all'.format(sa))
        scatter_err(directorySave, PRsetTest, sensorLocationSeed, runs,
                    error_array_1st_ff, '1st free flow',
                    error_array_2nd_ff, '2nd free flow', colors, 'Mean estimation error in free flow', 'err_summary_{0}_ff'.format(sa))
        scatter_err(directorySave, PRsetTest, sensorLocationSeed, runs,
                    error_array_1st_cf, '1st congested flow',
                    error_array_2nd_cf, '2nd congested flow', colors, 'Mean estimation error in congested flow', 'err_summary_{0}_cf'.format(sa))

        # plot the percent improvement
        scatter_perc_improve(directorySave, PRsetTest, sensorLocationSeed, runs, perc_improv_all, colors,
                             'Improvement of 2nd over 1st', 'err_summary_perc_{0}_all'.format(sa))
        scatter_perc_improve(directorySave, PRsetTest, sensorLocationSeed, runs, perc_improv_ff, colors,
                             'Improvement of 2nd over 1st in free flow', 'err_summary_perc_{0}_ff'.format(sa))
        scatter_perc_improve(directorySave, PRsetTest, sensorLocationSeed, runs, perc_improv_cf, colors,
                             'Improvement of 2nd over 1st in congested flow', 'err_summary_perc_{0}_cf'.format(sa))
